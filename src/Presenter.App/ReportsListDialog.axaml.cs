...
namespace Presenter
{
    public partial class ReportsListDialog : Window
    {
        public ReportsListDialog()
        {
            ...
            LibraryList.ItemsSource = new string[] { Labels.ReportsListOptionAll }.Union(Directory.GetDirectories(Config.LibraryPath).Select(p => System.IO.Path.GetFileName(p))).ToArray();
        }
        ...
        protected void Ok_Click(object sender, RoutedEventArgs e)
        {
            ...
            string origFilename = Path.Combine(desktop, Labels.ReportsListDocFilename + " - " + DateTime.Today.ToLongDateString() + ".rtf");
